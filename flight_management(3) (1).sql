...
BEGIN
    ...
    -- Insert the booking
    INSERT INTO bookings (flight_id, passenger_id, seat_number, class, price_paid, status)
    VALUES (p_flight_id, p_passenger_id, p_seat_number, p_class, v_price, 'Confirmed');
END //
DELIMITER ;

-- Procedure to get flight details with airport information
DELIMITER //
...
-- Procedure to get passenger booking history
DELIMITER //
...
-- Create a view for flight revenue
...
JOIN airlines al ON f.airline_id = al.airline_id
LEFT JOIN bookings b ON f.flight_id = b.flight_id
GROUP BY f.flight_id, f.flight_number, f.departure_time, f.arrival_time, al.airline_name;

-- Add indexes for commonly queried fields
CREATE INDEX idx_flight_dates ON flights(departure_time, arrival_time);
CREATE INDEX idx_booking_dates ON bookings(booking_date);
CREATE INDEX idx_passenger_name ON passengers(first_name, last_name);
CREATE INDEX idx_airport_city ON airports(city, country);

-- Example Queries

-- 1. CRUD Operations

-- SELECT: Get all flights from a specific airport
...
BEGIN
    ...
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Error occurred during transfer';
    END;

    START TRANSACTION;
    
    -- Get the old booking details
    SELECT @old_price := price_paid,
           @old_class := class,
           @old_passenger_id := passenger_id
    FROM bookings
    WHERE booking_id = p_booking_id;
    
    -- Get the new flight price
    SELECT 
        CASE @old_class
            WHEN 'Economy' THEN price_economy
            WHEN 'Business' THEN price_business
